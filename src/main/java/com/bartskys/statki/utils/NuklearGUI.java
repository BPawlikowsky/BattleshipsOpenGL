...
import static com.bartskys.statki.utils.IOUtil.*;
...
public class NuklearGUI {
      ...
      private int prog, vert_shdr, frag_shdr, uniform_tex, uniform_proj;

      private final Demo demo = new Demo();

      public NuklearGUI(long window, int width, int height) {
            this.win = window;
            this.width = width;
            this.height = height;
            try {
                  this.ttf = ioResourceToByteBuffer("res/FiraSans.ttf", 512 * 1024);
            } catch (IOException e) {
...
//                    });
      ...
      public void update(NkContext ctx, String str) {
            newFrame();
            demo.layout(ctx, 50, 50, str);
...
//            Objects.requireNonNull(ctx.clip().paste()).free();
      ...
      class Demo {
            ...
            private IntBuffer compression = createIntBuffer(1).put(0, 20);

            Demo() {
            }

            void layout(NkContext ctx, int x, int y, String string) {
                  try (MemoryStack stack = stackPush()) {
